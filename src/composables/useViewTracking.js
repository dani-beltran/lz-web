      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && (!trackOnce || !hasBeenViewed.value)) {
            ...
            // For tall sections, track when a meaningful portion is visible
            // For normal sections, use the standard threshold
            let shouldTrack = false
            
            if (elementHeight > viewportHeight) {
              // For tall sections: track when at least 300px or 30% of viewport is visible
              const minPixelsVisible = Math.min(300, viewportHeight * 0.3)
              shouldTrack = visibleHeight >= minPixelsVisible || visibleRatio >= 0.1
            } else {
              // For normal sections: use the configured threshold
              shouldTrack = visibleRatio >= threshold
            }
...
export function useAutoViewTracking(sectionName, options = {}) {
  const { hasBeenViewed, setupTracking, cleanup, reset } = useViewTracking(sectionName, options)
